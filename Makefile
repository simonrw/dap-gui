...
run: 			## Run the debugger
	cargo run $(RELEASE) --bin dap-gui-ui -- $(RUN_ARGS)
...
.PHONY: tui-attach
tui-attach:
	cargo r -p dap-tui $(RELEASE) -- launch.json -n Attach -b attach.py:29

.PHONY: tui-launch
tui-launch:
	cargo r -p dap-tui $(RELEASE) -- launch.json -n Launch -b test.py:4

.PHONY: profile-tui-launch
profile-tui-launch:
	cargo flamegraph --root -p dap-tui --release -- launch.json -n Launch -b test.py:4

.PHONY: repl-attach
repl-attach:
	cargo r -p repl $(RELEASE) -- launch.json -n Attach -b attach.py:29

.PHONY: repl-launch
repl-launch:
	cargo r -p repl $(RELEASE) -- launch.json -n Launch -b test.py:4
