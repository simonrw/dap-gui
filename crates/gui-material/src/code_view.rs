            ...
            if let Some((syn, theme)) = &highlighter {
                ...
                for (i, line) in s.as_str().lines().enumerate() {
                    ...
                    let bullet_color = if breakpoint_positions.contains(&line_num) {
                        Color32::from_rgb(242, 184, 181)
                    } else if hovered_gutter_line == Some(line_num) {
                        Color32::from_rgba_unmultiplied(242, 184, 181, 80)
                    } else {
                        Color32::TRANSPARENT
                    };
                    ...
                    for (style, text) in &regions {
                        ...
                        if is_current {
                            format.background = Color32::from_rgb(43, 41, 48);
                        }
                ...
                for (i, line) in s.as_str().lines().enumerate() {
                    ...
                    if highlight_line && i == current_line.wrapping_sub(1) {
                        layout_job.append(
                            ...
                            indent,
                            TextFormat {
                                background: Color32::from_rgb(43, 41, 48),
                                ..Default::default()
                            },
                        );
                    } else {
